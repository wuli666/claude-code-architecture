...
| 10 | [上下文经济学](./docs/10-context-economics.md) | Prompt Cache + ToolSearch + 延迟加载的合一架构、三层冷启动、Codex 对照 | cache_control、tool_reference、breakpoint、TTL |
...
### 6. Prompt Cache 驱动的延迟加载架构

```
工具 schema → 不进 tools 数组,进 messages 流(server-side tool_reference 展开)
              ↓
       cache_control breakpoint 沿前缀切段(每请求最多 4 个)
              ↓
       稳定段 1h TTL,易变段 5min TTL,长 TTL 段长期命中
```

CC 用 `tool_reference` 协议 + `cache_control` breakpoint 把 80+ 工具的 schema "驮"到对话增量里——deferred 工具**永远不进 tools 数组**,加载只是在 messages 流末尾追加,因此**工具集变动不污染长 TTL 的稳定缓存**。源码还有专门的 `promptCacheBreakDetection` 监控模块追踪 16+ 维度的 hash,生产数据显示 **77% 的 cache break 来自工具 description 内嵌动态内容**(详见 [10 - 上下文经济学](./docs/10-context-economics.md))。

这是 Anthropic API `cache_control` 能力与 ToolSearch 架构**协同设计**的产物——Codex 想抄也学不全,因为 OpenAI API 没给那么细粒度的缓存控制。
